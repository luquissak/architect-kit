...
# Mantemos DPI 600 para caso o usuário queira exportar para raster (PNG)
graph_attr["dpi"] = "600"

with Diagram("Plataforma MLOps SIPML - Referência Gartner v4", show=False, direction="LR", graph_attr=graph_attr, outformat="svg"):
    ...
    with Cluster("Fontes Externas de Dados"):
        ...
        with Cluster("Dados Operacionais (Produção)"):
            # Ajuste 1: SQL -> Storage e remoção de "SIARA"
            on_prem = Storage("On-Premises\n(IBM Db2 / Files)")
            # Ajuste 2: Storage -> Router e remoção de "Bacen / Serasa"
            external_apis = Router("API")
